...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-muted/40">
    ...
    <div className="md:pl-64"><header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b bg-background/90 px-4 backdrop-blur md:px-8"><div><p className="text-sm font-medium">Единая платформа</p><p className="text-xs text-muted-foreground">Данные vpn-platform-backend</p></div><div className="flex items-center gap-2"><Button type="button" variant="outline" size="sm" disabled={isLoggingOut} onClick={logout}><LogOut />{isLoggingOut ? "Выходим…" : "Выйти"}</Button><ThemeToggle /></div></header><main className="p-4 md:p-8">{children}</main></div>
  </div>;
}
