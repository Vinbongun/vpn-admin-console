...
// Код панели (REMNAWAVE/3X_UI) больше не редактируется отсюда — он уже виден в заголовке диалога,
// отдельное поле-инпут для него было чистым дублем. Для отдельных протоколов (не панель) поле code
// всё ещё рендерится в source-edit-dialog.tsx - пока нет никакой автоустановки, которая сама
// присваивала/чинила бы код, единственный способ его задать/поправить - вручную через эту форму.
// Optional здесь именно поэтому: значение всегда есть в defaultValues (source.code), просто не
// каждый рендер формы показывает для него инпут.
