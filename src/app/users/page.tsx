...
export default function CustomersPage() {
  ...
  return <AppShell><PageHeader title="Пользователи" description="Customers и brand memberships из OpenAPI 0.5.0" />
    ...
    <Card><CardHeader><CardTitle>Карточка клиента</CardTitle><CardDescription>`GET /admin/v1/customers/&#123;customerId&#125;`</CardDescription></CardHeader><CardContent>{!selectedId ? <p className="text-sm text-muted-foreground">Выберите клиента.</p> : detail.isLoading ? <p>Загрузка…</p> : detail.isError ? <p className="text-red-600">Не удалось получить карточку.</p> : detail.data && <div className="space-y-4"><div><p className="font-medium">{detail.data.email}</p><p className="text-xs text-muted-foreground">{detail.data.id}</p></div>{detail.data.memberships.map((membership) => <div className="rounded-lg border p-3" key={membership.id}><div className="flex items-center justify-between"><div><p className="font-medium">{membership.brandName}</p><p className="text-xs text-muted-foreground">{membership.brandCode}</p></div><Badge>{membership.status}</Badge></div><div className="mt-3 flex gap-2"><Button size="sm" variant="outline" disabled={membershipMutation.isPending} onClick={() => membershipMutation.mutate({ id: membership.id, status: "ACTIVE" })}>Активировать</Button><Button size="sm" variant="outline" disabled={membershipMutation.isPending} onClick={() => membershipMutation.mutate({ id: membership.id, status: "SUSPENDED" })}>Приостановить</Button></div></div>)}</div>}</CardContent></Card></div>
  </AppShell>;
}
