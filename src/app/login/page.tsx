...
export default function LoginPage() {
  ...
  return <main className="relative grid min-h-screen place-items-center bg-muted/40 p-4"><div className="absolute right-4 top-4"><ThemeToggle /></div><Card className="w-full max-w-md"><CardHeader className="items-center text-center"><div className="mb-2 rounded-xl bg-primary p-3 text-primary-foreground"><ShieldCheck /></div><CardTitle className="text-2xl">{step === "password" ? "VPN Platform" : "Подтвердите вход"}</CardTitle><CardDescription>{step === "password" ? "Вход сотрудника через admin API" : `Введите шестизначный код, отправленный на ${getValues("email")}`}</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={step === "password" ? submitPassword : submitOtp}>{step === "password" && <><div><label className="mb-1.5 block text-sm font-medium" htmlFor="email">Рабочий email</label><Input id="email" type="email" autoComplete="username" {...register("email")} />{errors.email && <p className="mt-1 text-xs text-red-600">{errors.email.message}</p>}</div><div><label className="mb-1.5 block text-sm font-medium" htmlFor="password">Пароль</label><Input id="password" type="password" autoComplete="current-password" {...register("password")} />{errors.password && <p className="mt-1 text-xs text-red-600">{errors.password.message}</p>}</div></>}{step === "otp" && <div><label className="mb-1.5 block text-sm font-medium" htmlFor="code">Код из письма</label><Input id="code" inputMode="numeric" autoComplete="one-time-code" autoFocus maxLength={6} placeholder="000000" {...register("code")} />{errors.code && <p className="mt-1 text-xs text-red-600">{errors.code.message}</p>}</div>}{requestError && <p className="rounded-lg bg-red-500/10 p-3 text-sm text-red-700 dark:text-red-400">{requestError}</p>}<Button className="w-full" disabled={isSubmitting} type="submit">{step === "password" ? "Продолжить" : "Подтвердить вход"}</Button>{step === "otp" && <Button className="w-full" type="button" variant="ghost" onClick={() => setStep("password")}>Вернуться к email и паролю</Button>}<p className="text-center text-xs text-muted-foreground">Сессия и permissions проверяются vpn-platform-backend.</p></form></CardContent></Card></main>;
}
