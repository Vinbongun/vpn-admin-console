...
export default function SubscriptionsPage() {
  ...
  return <AppShell><PageHeader title="Подписки" description="Управление подписками по OpenAPI 0.5.0" />
    ...
    <Card><CardHeader><CardTitle>Детали подписки</CardTitle><CardDescription>`GET/PATCH/DELETE /admin/v1/subscriptions/&#123;subscriptionId&#125;`</CardDescription></CardHeader><CardContent>{!selectedId ? <p className="text-sm text-muted-foreground">Выберите подписку.</p> : detail.isLoading ? <p>Загрузка…</p> : detail.isError ? <p className="text-red-600">Не удалось получить детали.</p> : detail.data && <div className="space-y-4"><div><p className="font-medium">{detail.data.customerEmail}</p><p className="text-xs text-muted-foreground">{detail.data.id}</p></div><dl className="grid grid-cols-2 gap-2 text-sm"><dt className="text-muted-foreground">Membership</dt><dd>{detail.data.brandMembershipId ?? "—"}</dd><dt className="text-muted-foreground">Plan ID</dt><dd>{detail.data.planId ?? "—"}</dd><dt className="text-muted-foreground">Revision</dt><dd>{detail.data.revision}</dd></dl>{mayWrite && <><select className="h-10 w-full rounded-lg border bg-background px-3 text-sm" value={editStatus} onChange={(event) => setEditStatus(event.target.value as Status)}>{["PENDING", "TRIAL", "ACTIVE", "PAST_DUE", "EXPIRED", "SUSPENDED", "REVOKED"].map((value) => <option key={value}>{value}</option>)}</select><Input placeholder="Причина изменения" value={reason} onChange={(event) => setReason(event.target.value)} /><div className="flex flex-wrap gap-2"><Button disabled={!reason || updateMutation.isPending} onClick={() => updateMutation.mutate({ id: selectedId, body: { status: editStatus, reason } })}>Сохранить</Button><Button variant="outline" disabled={rotateMutation.isPending} onClick={() => rotateMutation.mutate(selectedId)}>Ротировать token</Button><Button variant="outline" disabled={!reason || revokeMutation.isPending} onClick={() => revokeMutation.mutate({ id: selectedId, reason })}>Отозвать</Button></div></>}</div>}</CardContent></Card></div>
  </AppShell>;
}
