...
export default function SubscriptionsPage() {
  ...
  return <AppShell><PageHeader title="Подписки" description="Управление подписками клиентов" />
    {issuedToken && <Card className="mb-4 border-amber-500/50"><CardHeader><CardTitle>Новый token — сохраните сейчас</CardTitle><CardDescription>Plaintext возвращён backend один раз и не сохраняется интерфейсом.</CardDescription></CardHeader><CardContent><code className="block break-all rounded-lg bg-muted p-3 text-sm">{issuedToken.token}</code><Button className="mt-3" variant="outline" onClick={() => setIssuedToken(undefined)}>Скрыть</Button></CardContent></Card>}
    <Card className="mb-4"><CardHeader><CardTitle>Фильтры</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-3"><Input placeholder="Brand code" value={brandCode} onChange={(event) => { setBrandCode(event.target.value); setPage(1); }} /><Input placeholder="Customer email" value={customerEmail} onChange={(event) => { setCustomerEmail(event.target.value); setPage(1); }} /><Input placeholder="Status" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} /></CardContent></Card>
    {mayWrite && <Card className="mb-4"><CardHeader><CardTitle>Создать подписку</CardTitle><CardDescription>Membership и plan загружаются из backend.</CardDescription></CardHeader><CardContent><form className="grid gap-3 md:grid-cols-2 xl:grid-cols-5" onSubmit={submit}><select aria-label="Brand membership" className="h-10 rounded-lg border bg-background px-3 text-sm" {...form.register("brandMembershipId")}><option value="">Выберите membership</option>{customers.data?.items.flatMap((customer) => customer.memberships.map((membership) => <option key={membership.id} value={membership.id}>{customer.email} · {membership.brandCode}</option>))}</select><select aria-label="Plan" className="h-10 rounded-lg border bg-background px-3 text-sm" {...form.register("planId")}><option value="">Без тарифа</option>{plans.data?.items.map((plan) => <option key={plan.id} value={plan.id}>{plan.brandCode} · {plan.name}</option>)}</select><select className="h-10 rounded-lg border bg-background px-3 text-sm" {...form.register("status")}><option value="PENDING">PENDING</option><option value="TRIAL">TRIAL</option><option value="ACTIVE">ACTIVE</option></select><Input aria-label="Начало" type="datetime-local" {...form.register("startsAt")} /><Input aria-label="Окончание" type="datetime-local" {...form.register("expiresAt")} /><Button disabled={createMutation.isPending} type="submit">Создать</Button></form>{Object.keys(form.formState.errors).length > 0 && <p className="mt-3 text-sm text-red-600">Проверьте обязательные поля.</p>}</CardContent></Card>}
    <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]"><Card><CardHeader><CardTitle>Список подписок</CardTitle><CardDescription>`GET /admin/v1/subscriptions`</CardDescription></CardHeader><CardContent className="overflow-x-auto">{subscriptions.isLoading ? <p>Загрузка…</p> : subscriptions.isError ? <p className="text-red-600">Не удалось получить подписки.</p> : <><table className="w-full min-w-3xl text-sm"><thead><tr className="border-b text-left"><th className="p-3">Клиент</th><th className="p-3">Бренд</th><th className="p-3">Тариф</th><th className="p-3">Статус</th><th className="p-3">До</th><th /></tr></thead><tbody>{subscriptions.data?.items.map((item) => <tr className="border-b last:border-0" key={item.id}><td className="p-3">{item.customerEmail ?? "—"}</td><td className="p-3">{item.brandCode}</td><td className="p-3">{item.planCode ?? "—"}</td><td className="p-3"><Badge>{item.status}</Badge></td><td className="p-3">{new Date(item.expiresAt).toLocaleString("ru-RU")}</td><td className="p-3"><Button size="sm" variant="outline" onClick={() => { setSelectedId(item.id); setEditStatus(item.status); }}>Подробнее</Button></td></tr>)}</tbody></table><div className="mt-4 flex items-center justify-between"><Button variant="outline" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Назад</Button><span className="text-sm text-muted-foreground">{page} / {totalPages}</span><Button variant="outline" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}>Далее</Button></div></>}</CardContent></Card>
    <Card><CardHeader><CardTitle>Детали подписки</CardTitle><CardDescription>`GET/PATCH/DELETE /admin/v1/subscriptions/&#123;subscriptionId&#125;`</CardDescription></CardHeader><CardContent>{!selectedId ? <p className="text-sm text-muted-foreground">Выберите подписку.</p> : detail.isLoading ? <p>Загрузка…</p> : detail.isError ? <p className="text-red-600">Не удалось получить детали.</p> : detail.data && <div className="space-y-4"><div><p className="font-medium">{detail.data.customerEmail}</p><p className="text-xs text-muted-foreground">{detail.data.id}</p></div><dl className="grid grid-cols-2 gap-2 text-sm"><dt className="text-muted-foreground">Membership</dt><dd>{detail.data.brandMembershipId ?? "—"}</dd><dt className="text-muted-foreground">Тариф</dt><dd>{detail.data.planCode ?? "Без тарифа"}</dd><dt className="text-muted-foreground">Revision</dt><dd>{detail.data.revision}</dd></dl>{mayWrite && <><select ref={planSelectRef} key={detail.data.id} aria-label="Тариф" className="h-10 w-full rounded-lg border bg-background px-3 text-sm" defaultValue={detail.data.planId ?? ""}><option value="">Без тарифа</option>{plans.data?.items.map((plan) => <option key={plan.id} value={plan.id}>{plan.brandCode} · {plan.name}</option>)}</select><select aria-label="Статус подписки" className="h-10 w-full rounded-lg border bg-background px-3 text-sm" value={editStatus} onChange={(event) => setEditStatus(event.target.value as Status)}>{["PENDING", "TRIAL", "ACTIVE", "PAST_DUE", "EXPIRED", "SUSPENDED", "REVOKED"].map((value) => <option key={value}>{value}</option>)}</select><Input placeholder="Причина изменения" value={reason} onChange={(event) => setReason(event.target.value)} /><div className="flex flex-wrap gap-2"><Button disabled={!reason || updateMutation.isPending} onClick={() => updateMutation.mutate({ id: selectedId, body: { status: editStatus, planId: planSelectRef.current?.value || undefined, reason } })}>Сохранить</Button><Button variant="outline" disabled={rotateMutation.isPending} onClick={() => rotateMutation.mutate(selectedId)}>Ротировать token</Button><Button variant="outline" disabled={!reason || revokeMutation.isPending} onClick={() => revokeMutation.mutate({ id: selectedId, reason })}>Отозвать</Button></div></>}</div>}</CardContent></Card></div>
  </AppShell>;
}
